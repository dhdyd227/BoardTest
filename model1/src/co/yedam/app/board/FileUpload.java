...
public class FileUpload implements Command{
	...
	public String excute(HttpServletRequest request, HttpServletResponse response)
			...
			String uploadPath = request.getSession().getServletContext().getRealPath("/images"); // upload는 폴더명 / 폴더의 경로를 구해옴
			...
			try {
				...
				*/
				...
				String fileName1 = multi.getFilesystemName("uploadfile"); // name=uploadfile의 업로드된 시스템 파일명을 구함(중복된 파일이 있으면, 중복 처리 후 파일 이름)
				String orgfileName1 = multi.getOriginalFileName("uploadfile"); // name=uploadfile의 업로드된 원본파일 이름을 구함(중복 처리 전 이름)
